...
let currentLeaderboardMode = "total";
...
async function initMap() {
  ...
  if (window.currentUser && typeof window.loadUserData === "function") {
...
function isShowNameOnLeaderboard() {
  return localStorage.getItem("showNameOnLeaderboard") === "true";
}

function setupLeaderboardNameToggle() {
  const toggle = id("showNameOnLeaderboardToggle");
  if (!toggle) return;

  toggle.checked = isShowNameOnLeaderboard();
...
function openProfile() {
  ...
  const riverCount = Object.keys(state.stats.visitedRivers).length;
  const quizScore = state.stats.quizScore || 0;
  const warningCount = state.stats.warnings || 0;

  const progress = Math.min(Math.round((riverCount / 5) * 100), 100);

  let levelTitle = "Lv.1 河岸新手";
  let levelDesc = "開始探索大台北水脈吧！";

  if (badgeCount >= 15 || riverCount >= 5) {
    levelTitle = "Lv.5 水脈達人";
    levelDesc = "你已經快把大台北水系摸熟了，超派。";
  } else if (badgeCount >= 10 || riverCount >= 4) {
    levelTitle = "Lv.4 城市導覽家";
    levelDesc = "你已具備完整的河岸文化探索能力。";
  } else if (badgeCount >= 5 || riverCount >= 3) {
    levelTitle = "Lv.3 河流探索者";
    levelDesc = "你正在把地圖變成自己的城市記憶。";
  } else if (badgeCount >= 2 || riverCount >= 1) {
    levelTitle = "Lv.2 水岸旅人";
    levelDesc = "已開始累積探索紀錄，繼續衝。";
  }

  id("profileUserBox").innerHTML = `
    <div class="user-profile-box">
      <img src="${user.photoURL || "icon/user.png"}" class="user-avatar" alt="使用者頭像">
      <div>
        <h3>👋 歡迎回來，${user.displayName || "探索者"}</h3>
        <p>${user.email || ""}</p>
        <button onclick="window.logout()" class="ghost-btn">登出</button>
      </div>
    </div>
  `;

  id("profileBadgeCount").textContent = badgeCount;
  id("profileRiverCount").textContent = `${riverCount} / 5`;
  id("profileQuizScore").textContent = quizScore;
  id("profileWarningCount").textContent = warningCount;

  id("profileLevelTitle").textContent = levelTitle;
  id("profileLevelDesc").textContent = levelDesc;
  id("profileProgressText").textContent = `${progress}%`;
  id("profileProgressFill").style.width = `${progress}%`;

  const badgeBoard = id("profileBadgeBoard");
  badgeBoard.innerHTML = badgeCount
    ? ""
    : `<span class="empty-badge">尚未取得徽章。</span>`;
  ...
  id("profileRecentBadges").innerHTML = recent.length
    ? recent.map(x => `
        <div class="recent-badge-item">
          <span>🏅</span>
          <div>
            <b>${x.name}</b>
            <small>${x.acquiredAt ? new Date(x.acquiredAt).toLocaleDateString("zh-TW") : ""}</small>
          </div>
        </div>
      `).join("")
    : `<p class="empty-badge">尚未取得徽章。</p>`;

  setupLeaderboardNameToggle();
  renderLeaderboard();
  openModal("profileModal");
}

async function renderLeaderboard(mode = currentLeaderboardMode) {
  currentLeaderboardMode = mode;

  const box = id("leaderboardList");
  const subtitle = id("leaderboardSubtitle");

  if (!box || typeof window.loadLeaderboard !== "function") return;
  ...
  document.querySelectorAll(".leaderboard-tab").forEach(btn => {
    btn.classList.toggle("active", btn.dataset.board === mode);
  });

  box.innerHTML = `<p class="empty-badge">排行榜載入中...</p>`;

  try {
    const rows = await window.loadLeaderboard(mode);

    box.innerHTML = rows.length
      ? rows.map((u, index) => {
        const totalScore =
          u.totalScore ??
          (u.quizScore + u.badgeCount * 20 + u.riverCount * 50);
        ...
        return `
          <div class="leaderboard-item">
            ...
            <div class="leaderboard-main">
              <b>${u.displayName}</b>
              <small>
                綜合 ${totalScore}｜分數 ${u.quizScore}｜徽章 ${u.badgeCount}｜探索河流 ${u.riverCount}/5
              </small>
            </div>
      ...
      : `<p class="empty-badge">尚無排行榜資料。</p>`;

  } catch (err) {
    console.error("排行榜載入失敗：", err);
    box.innerHTML = `<p class="empty-badge">排行榜載入失敗，請看 Console。</p>`;
  }
}
